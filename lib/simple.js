 ...
 */
 exports.smsg = (conn, m, hasParent) => {
    ...
    m.reply = async (text, chatId, options) => {
    	let pp = await conn.profilePictureUrl(m.sender, 'image').catch(_ => global.pp)
        let { data } = await conn.getFile(await(await require('node-fetch')(pp)).buffer())
        conn.reply(chatId ? chatId : m.chat, text, m, { contextInfo: { mentionedJid: conn.parseMention(text), externalAdReply: { title: `N O T I F I C A T I O N`, thumbnail: data }}, options })
    }    
...
conn.loading = async (jid, loamsg, loamsgEdit, loadingMessages, imageUrl,quoted,  options) => {
    try {
        // Fetch the image from the URL
        const response = await axios.get(imageUrl, { responseType: 'arraybuffer' });
        const imageBuffer = Buffer.from(response.data, 'binary'); // Convert the response data to a Buffer

        // Send the initial loading message with the image
        let { key } = await conn.sendMessage(jid, { image: imageBuffer, caption: loamsg, ...options }, { quoted });
        ...
        await conn.sendMessage(jid, { text: "Failed to load image.", ...options }, { quoted });
    }
};

/** UNTUK POLLING**/

conn.poll = async (jid, title = "", a, options = {}) => {
if (!Array.isArray(a)) {
throw new Error("Parameter 'a' harus berupa array.");
}
return await conn.sendMessage(jid, {
poll: {
name: title,
