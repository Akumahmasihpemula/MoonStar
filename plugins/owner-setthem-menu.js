...
ᴘᴀʏᴍᴇɴᴛ
ᴇᴅɪᴛ
> thema sukses di set ke awal  `;
  global.menu = text;
  if (text === "simple") {
    m.reply(`THEMA SUDAH dI ALIHKAN KE SIMPLE`);
  } else if (text === "image") {
    m.reply(`THEMA SUDAH dI ALIHKAN KE IMAGE`);
  } else if (text === "video") {
    m.reply(`THEMA SUDAH dI ALIHKAN KE VIDEO`);
  } else if (text === "payment") {
    m.reply(`THEMA SUDAH dI ALIHKAN KE  PAYMENT`);
  } else if (text === "edit") {
    m.reply(`THEMA SUDAH dI ALIHKAN KE EDIT`);
  } else {
 m.reply(des) 
  }
};
