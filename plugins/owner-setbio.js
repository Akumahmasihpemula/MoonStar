...
if (!isOwner) return m.reply("command tersbut hanya bisa di akses oleh owner dan bot") 
if (!text) return m.reply(`[ PERINTAH ]\n> Contoh: ${command} sedang gabut`);
await conn.updateProfileStatus(text);    
m.reply(`[ BIO SUKSES ]\n> bio ${namebot} sukses di update bio baru "${text}"`)   
};
handler.help = ["setbio"];
handler.tags = ["owner"];
