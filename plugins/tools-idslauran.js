...
if (!m.quoted) throw `*[ PENGGUNAAN FITURE ]*\n> reply link saluran undangan admin dengan gunakan commend ${command} `
const idsaluran = m.quoted.newsletterJid;
const namesaluran = m.quoted.newsletterName;
const cap = `
ID: ${idsaluran}
NAMA: ${namesaluran}`;
m.reply(cap);
};
handler.help = ['idsaluran <reply link admin>'];
handler.tags = ['tools'];
handler.command = /^(idsaluran|idkom|idch)$/i;
handler.limit = 8
