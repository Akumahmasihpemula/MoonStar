let handler = async (m, { conn, text }) => {
    if (!text) throw '[ FITUR HAPUS OWNER ]\n> Tolong tag owner yang mau dihapus.';
    
    let who;
    if (m.isGroup) {
        who = m.mentionedJid[0]; // Mengambil ID pengguna yang ditandai
    } else {
        who = m.chat; // Jika bukan grup, gunakan chat ID
    }

    if (!who) throw '[ FITUR HAPUS OWNER ]\n> Tag pengguna yang akan dihapus.';

    let users = global.db.data.users;

    // Pastikan pengguna ada dalam database
    if (!users[who]) throw '[ FITUR HAPUS OWNER ]\n> Pengguna tidak ditemukan dalam database.';

    // Menghapus status owner
    users[who].owner = false;

    // Mengirimkan balasan ke chat
    conn.reply(m.chat, `[ SUKSES PENGHAPUSAN ]\n> Selesai menghapus owner fake ${who}`, m);
};
