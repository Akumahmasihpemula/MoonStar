...
let handler = async (m, { conn, text, command }) => {
    if (!text) return m.reply(`Masukkan input judul lagu\n\nContoh: .play kenangan`);
    m.reply("\`Tunggu 3 detik untuk menjalankan perintah\`");

    try {
        let result = await yts.search(text);
        let video = result.videos[Math.floor(Math.random() * result.videos.length)];
        let data = await ytmp4(video.url);

        const { title, views } = video; // Mengambil title dan views dari video
        const cap = `[ PENCARIAN YOUTUBE VIDEOS ]\n> Judul: ${title}\n> Jumlah Views: ${views}`;

        conn.sendMessage(m.chat, { 
            video: { url: data.dl }, // Pastikan data.url ada
            fileName: title + '.mp4', 
            mimetype: 'video/mp4', 
            caption: cap 
        }, { quoted: m });
