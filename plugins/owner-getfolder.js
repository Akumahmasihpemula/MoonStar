...
let handler = async (m, { text, conn }) => {
  if (!text) return m.reply("Nama foldernya apa ?");
  let mimetype = "application/zip";
  let name = `${text}.zip`;
  let jpegThumbnail = fs.readFileSync("./thumbnail.jpg");
  let folder = await fs.existsSync("./" + text);
  if (!folder) return m.reply("Folder tidak di temukan");
  m.reply(wait);
  exec(`zip -r ${name} ${text}`, async (error, stdout, stderr) => {
    if (error) {
