             ...
             if (!isNumber(user.ayam)) user.ayam = 0
                    ...
                    if (!isNumber(user.skata)) user.skata = 0
                        ...
                        if (!isNumber(user.premiumDate)) user.premiumDate = -1
                         if (!isNumber(user.vipDate)) user.vipDate = -1
...
if (!('sDemote' in chat)) chat.sDemote = ''
                ...
                if (!('resetlimit' in chat)) chat.resetlimit = true
...
sDemote: '',
            ...
            if (opts['nyimak'] && !global.db.data.users[m.sender].premium && !global.db.data.users[m.sender].vip && !global.db.data.users[m.sender].owner) return
if (!m.fromMe && opts['self'] && !global.db.data.users[m.sender].premium && !global.db.data.users[m.sender].vip && !global.db.data.users[m.sender].owner) return
if (opts['pconly'] && m.chat.endsWith('g.us') && !global.db.data.users[m.sender].premium && !global.db.data.users[m.sender].vip && !global.db.data.users[m.sender].owner) return
if (opts['gconly'] && !m.fromMe && !m.chat.endsWith('g.us') && !global.db.data.users[m.sender].premium && !global.db.data.users[m.sender].vip && !global.db.data.users[m.sender].owner) return conn.sendMessage(m.chat, {
text: "[ GROUP ONLY ]\n> hai kak maaf saya tidak menanggapi pengguna karena saya dalam mode group yang hanya bisa di gunakan di dalam group jika mau chat privat kamu bisa berlangganan premium dari 7k-25k kamu bisA cek harga dan akses guna kamu bisa menggunakan comand (.sewa) ",
contextInfo: {
externalAdReply: {
title: 'M O D E G R O U P',
thumbnailUrl:pp, 
mediaType: 1,
renderLargerThumbnail: false
        ...
        if (m.id.startsWith('BAE5') && m.id.length === 16 || m.isBaileys && m.fromMe) return
            ...
            let isVip = isROwner || (db.data.users[m.sender].vipTime > 0 || db.data.users[m.sender].vip)
      ...
      let match = (_prefix instanceof RegExp ? [
        ...
        [
          [], new RegExp()
                ...
                // Assuming `plugin` is defined and contains the command logic
...
// Determine if the message is accepted based on the prefix options
let isAcc = opts.singleprefix && (match && match[0] !== null) || 
            (opts.multiprefix || opts.noprefix) && (opts && match && m);

// If the message is accepted, process it
if (isAcc) {
    let result = null;

    // Determine the used prefix based on the options
    if (opts.noprefix && !opts.singleprefix && !opts.multiprefix) {
        result = match ? (match[0] || "")[0] : null;
    } else if (opts.multiprefix || !opts.noprefix) {
        result = match ? (match[0] || "")[0] : null;
    } else if (opts.singleprefix && !opts.noprefix && !opts.multiprefix) {
        result = match ? (match[0] || "")[0] : null;
    }
    ...
    // Check if the command is accepted based on the prefix
    let isAccept = opts.singleprefix && 
               ...
               if (!isAccept) continue;
                 m.plugin = name;
                    ...
                    if (plugin.rowner && !isROwner) { // Real Owner
                     ...
                     if (plugin.gcbot && !m.chat.startsWith('120363164510479075')) {
                     fail('gcbot', m, this);
                     continue
                     }
                    if (plugin.nsfw && !m.chat.startsWith('120363330122320984')) {
                     fail('nsfw', m, this);
                      continue;
                       }
                    ...
                    if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                        this.reply(m.chat, `_*[ LIMIT HABIS ]*_\n> kamu tidak dapat menggunakan fitur: ${command} karna limit kamu sudah habis `, m)
                        continue // Limit habis
                    }
                    if (plugin.level > _user.level) {
                        this.reply(m.chat, `_*[ AKSES DI TOLAK ]*_\n> di butuhkan level ${plugin.level} level kamu di bawah nya level kamu${_user.level} untuk menggunakan command ${command} kamu harus menaikan level terlebih dahulu`, m)
                    ...
                    try {
                        ...
                        if (e) {
                            ...
                            for (let jid of owner.map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').filter(v => v != this.user.jid)) {
                                ...
                                if (data.exists)
                                    m.reply(`\`[ S I S T E M E R O R ]\`

⚠︎ ᴘʟᴜɢɪɴ:\`${m.plugin}\`
⚠ ᴜsᴇʀ: \`${await this.getName(m.chat)}\`
︎⚠ *ᴄᴏᴍᴍᴀɴᴅ:* \`${usedPrefix}${command} ${args.join(' ')}\`\n\n\`\`\`${text}\`\`\``.trim(), data.jid, { mentions: [m.sender] })
                            }
                            m.reply(text)
                        }
                    } finally {
                        ...
                        if (m.limit) m.reply(`*_[ PENGGUNAAN LIMIT ]_*\n> senpai menggunakan ${m.limit} limit\n\`SISA LIMIT SENPAI ${_user.limit}\``)
                   }
                    break
                }
            }
 ...
 if (global.setwelcome === 'simple') {  
 conn.reply(id,text.trim(),global.fkontak) 
} else if (global.setwelcome === 'image') { 
let wel = await drawCard({
    ...
    background: 'https://qu.ax/CWhQo.jpg',
    blur: 1,
    border: true,
    rounded: true,
  })
...
} else if (global.setwelcome === 'payment') {    
await conn.relayMessage(id,  {
 ...
 this.sendFile(id, action === 'add' ? ppgc : ppgc, 'pp.jpg', text, null, false, { mentions: [user] })
                    }
                }
            }
        }
    ...
    async delete({fromMe, id, participant }) {
        ...
        let chat = global.db.data.chats[msg.key.id] || {}
        if (chat.delete) return
        await this.reply(msg.key.id, `
...
`.trim(), msg, {
        ...
        this.copyNForward(msg.key.id, msg).catch(e => console.log(e, msg))
    }
}

global.dfail = (type, m, conn) => {
const msg = {
owner:` 「 OWNER ONLY 」\n> fitur tersebut hanya bisa di gunakan oleh pemilik ${namebot}`, 
rowner:` 「 CREATOR ONLY 」\n> fitur tersebut hanya bisa di gunakan oleh creator bot`, 
mods:` 「 MEDERATOR ONLY 」\n> fitur tersebut hanya bisa di gunakan oleh MEDERATOR bot`, 
vip:` 「 PREMIUM ONLY 」\n>hanya bisa di akses oleh pengguna PREMIUM`, 
premium:` 「 PREMIUM ONLY 」\n> hanya bisa di akses oleh pengguna PREMIUM`, 
admin:` 「 ADMIN ONLY 」\n> kamu di larang akses fitur tersebut hanya tersedia oleh admin group`, 
group:` 「 GROUP ONLY 」\n> kamu di larang akses fitur tersebut hanya tersedia oleh GROUP BOT`, 
botAdmin:`「BOTADMIN ONLY 」\n> mohon jadikan saya admin terlebih dahulu untuk mengakses fitur tersebut`, 
private:`「PRIVAT ONLY 」\n> senpai baka hanya bisa di gunakan di dalam ${namebot} tidak bisa di group`, 
nsfw:`「 NSFW ONLY 」\n> fitur tersebut hanya bisa di akses di group nsfw join terlebih dahulu di url: https://chat.whatsapp.com/EN9EeZGvwSAGoJrJBz8NsX`, 
gcbot:`「 GCBOT ONLY 」\n> akses fitur tersebut hanya bisa di akses di group pribadi jin untuk akses url: https://chat.whatsapp.com/GuUZnMCE9wf3BSkw1nmFD1`, 
unreg:`_*\`[ TERDETEKSI BELUM TERDAFTAR ]\`*_

\`silahkan registrasi terlebih dahulu agar data kamu tersimpan di dalam *DATABASE* ${namebot} cara pendaftaran ikutin langkah-langkah di bawah ini pilih salah satu yang menurut kamu simple dan kamu ketshui contoh seperti di bawah ini\`

╭─ •「 *LIST PENDAFTARAN*」
│
│○ \`biasa\`: .daftar nama.umur.kelamin.kota
│○ \`otomatis\`: @verify
│○ \`via name\`: .regname nama mu
│○ \`via email\`: .regmail (sedang tidak sedia) 
╰───────────────`
 }[type]
